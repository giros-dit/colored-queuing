...
/*
 * net/sched/sch_cdrr.c Colored-DRR scheme.
 *
...
#define MODULE_ALIAS_NET_SCH(name) MODULE_ALIAS("qdisc-" name)
#endif
...
#define DRR_DEFAULT_TMAX 1500 // MTU (1500)
#define DRR_DEFAULT_MODE 0 // Default mode: threshold dropping
#define TCA_MYDRR_MAX	(__TCA_MYDRR_MAX - 1)

extern const struct Qdisc_ops bfifo_cc_qdisc_ops;
...
struct drr_class {
	...
	u32				quantum;
	u32				deficit;
    u32             tmax; // Maximum queue packet size
	u32 			mode; // 0 for threshold dropping, 1 for growing-detection dropping
};
...
struct bfifo_cc_sched_data {
	...
	u32 queue_control; // Control variable to determine when to drop packets. In threshold dropping mode, this is the threshold. In growing-detection dropping mode.
...
static const struct nla_policy drr_policy[TCA_MYDRR_MAX + 1] = {
	[TCA_MYDRR_QUANTUM]	= { .type = NLA_U32 },
    [TCA_MYDRR_TMAX]    = { .type = NLA_U32 },
	[TCA_MYDRR_MODE]    = { .type = NLA_U32 },
};
			    ...
			    struct netlink_ext_ack *extack)
{
	...
	struct nlattr *tb[TCA_MYDRR_MAX + 1];
	u32 quantum;
    u32 tmax;
	u32 mode;
	int err;
	...
	err = nla_parse_nested_deprecated(tb, TCA_MYDRR_MAX, opt, drr_policy,
					  extack);
	if (err < 0)
		return err;

	if (tb[TCA_MYDRR_QUANTUM]) {
		quantum = nla_get_u32(tb[TCA_MYDRR_QUANTUM]);
		if (quantum == 0) {
    ...
    if (tb[TCA_MYDRR_TMAX]) {
		tmax = nla_get_u32(tb[TCA_MYDRR_TMAX]);
        if (tmax <= 42) {
			NL_SET_ERR_MSG(extack, "Specified DRR tmax must be greater than 42");
			return -EINVAL;
        }
	} else
        tmax = DRR_DEFAULT_TMAX;

	if (tb[TCA_MYDRR_MODE]) {
		mode = nla_get_u32(tb[TCA_MYDRR_MODE]);
        if (mode > 1 || mode < 0) {
			NL_SET_ERR_MSG(extack, "Specified DRR mode is invalid");
			return -EINVAL;
        }
	} else
		mode = DRR_DEFAULT_MODE;
			   ...
			   struct netlink_ext_ack *extack)
{
	...
	*old = qdisc_replace(sch, new, &cl->qdisc);
	...
	if (cl->qdisc->ops == &bfifo_cc_qdisc_ops) {
		struct bfifo_cc_sched_data *c = qdisc_priv(cl->qdisc);
		WRITE_ONCE(c->tmax, cl->tmax);
		WRITE_ONCE(c->mode, cl->mode);
		if (cl->mode == 1){
			WRITE_ONCE(c->queue_control, 0);
			WRITE_ONCE(c->deficit, cl->quantum);
		}
			  ...
			  struct sk_buff *skb, struct tcmsg *tcm)
{
	...
	if (nla_put_u32(skb, TCA_MYDRR_QUANTUM, cl->quantum))
		goto nla_put_failure;
	
	if (nla_put_u32(skb, TCA_MYDRR_TMAX, cl->tmax))
		goto nla_put_failure;

	if (nla_put_u32(skb, TCA_MYDRR_MODE, cl->mode))
		goto nla_put_failure;
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Colored Based Deficit Round Robin scheduler");
